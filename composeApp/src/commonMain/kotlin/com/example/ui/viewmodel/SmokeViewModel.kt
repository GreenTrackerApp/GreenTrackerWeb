...
fun String.translate(lang: String): String {
    ...
    return when (this) {
        ...
        "Journal Multi-Photo: Attach up to 3 photos per strain entry" -> "Journal Multi-Foto: Bis zu 3 Fotos pro Sorteneintrag anfügen"
        "Cover Photo & Favorite: Set any photo as the primary cover photo" -> "Titelbild & Favorit: Jedes Foto als Haupt-Titelbild festlegbar"
        "Fullscreen Gallery: Swipe through photos with thumbnail preview strip" -> "Vollbild-Galerie: Fotos wischen mit Miniaturbild-Vorschau"
        "Strain Share & Business Card: Share strain entries as digital business cards" -> "Sorten-Visitenkarte: Sorten als digitale Visitenkarte teilen"
        "Theme Warning Colors: Amber warning indicator when reaching daily limit" -> "Theme-Warnfarben: Amber-Warnanzeige beim Erreichen des Tageslimits"
        "Auto Language Detection: System language detection on first launch" -> "Automatische Spracherkennung: System-Spracherkennung beim ersten Start"
        "Share Strain" -> "Sorte teilen"
        "Strain Business Card" -> "Sorten-Visitenkarte"
        "Copy Card Summary" -> "Zusammenfassung kopieren"
        "Download Card" -> "Visitenkarte herunterladen"
        "Cover Photo" -> "Titelbild"
        "Set as Cover Photo" -> "Als Titelbild festlegen"
        "Switch" -> "Wechseln"
        "Change Cover Photo?" -> "Titelbild ändern?"
        "Set this photo as the new favorite cover photo?" -> "Dieses Foto als neues Lieblings-Titelbild festlegen?"
        "Delete Photo?" -> "Foto löschen?"
        "Do you really want to delete this photo?" -> "Möchtest du dieses Foto wirklich löschen?"
        "Discard changes?" -> "Änderungen verwerfen?"
        "You have unsaved changes. Do you want to discard them?" -> "Du hast ungespeicherte Änderungen. Möchtest du sie verwerfen?"
        "Discard" -> "Verwerfen"
        "Keep Editing" -> "Weiter bearbeiten"
        "Max 3 Photos" -> "Maximal 3 Fotos"
        else -> this
    }
}
