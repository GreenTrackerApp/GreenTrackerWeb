...
@JsFun("(title, body) => { if ('serviceWorker' in navigator) { navigator.serviceWorker.ready.then(registration => { registration.showNotification(title, { body: body, icon: 'icon-192.png', badge: 'icon-192.png', vibrate: [200, 100, 200] }); }).catch(err => { console.error('SW Notification error:', err); if (typeof Notification !== 'undefined') { new Notification(title, { body: body }); } }); } else if (typeof Notification !== 'undefined') { try { new Notification(title, { body: body }); } catch(e) { console.error('Legacy notification error:', e); } } }")
